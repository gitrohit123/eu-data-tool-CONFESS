...
import { AlignmentColors } from "./MultiColorCircularProgress";
...
const AlignmentCard = ({
  title,
  alignedActivitiesTotal,
  notAlignedActivitiesTotal,
  notEligibleActivitiesTotal,
  total,
  showEuro,
}: Props) => {
  return (
    <Card className={"mt-[30px] w-[290px] h-[300px]"}>
      <CardBody>
        ...
        >
          ...
          <div>
            {alignedActivitiesTotal} {showEuro ? "€" : ""} (
            {Math.round((alignedActivitiesTotal / total) * 100)}%)
          </div>
        </div>
        <div
          className={"text-xs"}
        ...
        >
          ...
          <div>
            {notAlignedActivitiesTotal} {showEuro ? "€" : ""} (
            {Math.round((notAlignedActivitiesTotal / total) * 100)}%)
          </div>
        </div>
        <div
          className={"text-xs"}
        ...
        >
          ...
          <div>
            {notEligibleActivitiesTotal} {showEuro ? "€" : ""} (
            {Math.round((notEligibleActivitiesTotal / total) * 100)}%)
          </div>
        </div>
      </CardBody>
    </Card>
